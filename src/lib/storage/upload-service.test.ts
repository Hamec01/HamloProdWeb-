...
test("createUploadUrl rejects an unauthenticated caller", async () => {
  const result = await createUploadUrl({ isAuthorized: false, sameOrigin: true, body: {}, storage: fakePort() });
  assert.equal(result.status, 401);
  assert.equal(result.body.error, "Unauthorized");
});

test("createUploadUrl rejects a foreign origin before auth / S3", async () => {
...
test("finalizeUpload rejects a foreign origin before auth / S3", async () => {
  const storage = fakePort({ head: { contentType: "image/png", contentLength: 2 * MB, etag: '"e"' } });
  const result = await finalizeUpload({
    isAuthorized: true,
    sameOrigin: false,
    storage,
    body: { key: `beats/${BEAT_ID}/cover/aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee.png`, kind: "beat-cover" },
  });
  assert.equal(result.status, 403);
});
...
test("createUploadUrl issues a presigned PUT for a valid beat cover request", async () => {
  const storage = fakePort();
  const result = await createUploadUrl({
    isAuthorized: true, sameOrigin: true,
    storage,
    body: { kind: "beat-cover", entityId: BEAT_ID, originalFileName: "cover.png", contentType: "image/png", size: 2 * MB },
  });
...
test("finalizeUpload rejects an unauthenticated caller", async () => {
  const result = await finalizeUpload({ isAuthorized: false, sameOrigin: true, body: {}, storage: fakePort() });
  assert.equal(result.status, 401);
});
